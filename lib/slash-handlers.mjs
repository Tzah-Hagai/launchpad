...
const LAUNCHPAD_HEADER =
  "*Launchpad* — quick links for this channel:";

async function pinMessage(channelId, messageTs) {
    ...
    blocks: [
      ...
      },
      {
        ...
        element: {
          ...
          placeholder: { type: "plain_text", text: "https://spinomenal.atlassian.net/wiki/..." },
        },
      },
      {
        type: "input",
        ...
        element: {
          type: "plain_text_input",
          action_id: "jira_url",
          placeholder: { type: "plain_text", text: "https://spinomenal.atlassian.net/jira/..." },
        },
      },
    ],
  };
...
async function postLaunchpad(channelId, mondayUrl, designUrl, jiraUrl) {
  ...
  const pinError = await pinMessage(channelId, data.ts);
  return { pinError };
}
...
export async function handleSlashCommand(rawBody, headers) {
  ...
  if (tokens.length === 3 && tokens.every(isValidUrl) && channelId) {
    ...
    try {
      const { pinError } = await postLaunchpad(channelId, mondayUrl, designUrl, jiraUrl);
      return {
        statusCode: 200,
        ...
        body: JSON.stringify({
          ...
          text: pinError
            ? `Launchpad buttons posted, but pinning failed (${pinError}). Add scope \`pins:write\` and reinstall, or pin manually.`
            : "Launchpad buttons posted and pinned to this channel.",
        }),
      };
    } catch (err) {
      return {
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      text:
        "*Launchpad*\n\n" +
        "• Run `/launchpad` with **no text** to open a form (Monday, Design, JIRA URLs).\n" +
        "• Or paste **three URLs** in one line:\n" +
        "`/launchpad https://monday.com/... https://spinomenal.atlassian.net/wiki/... https://spinomenal.atlassian.net/jira/...`",
    }),
  };
}
